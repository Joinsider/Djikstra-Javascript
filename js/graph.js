class Graph {
    ...
    addNodes(key, value, cost) {
        ...
        if (key.toString() === "" || value.toString() === "") {
            alert("Please check your Connections, you might entered empty connections");
        } else if (cost < 1) {
            alert("Please enter a number larger or equal to 1.")
            return false;
        } else if (this.nodes.has(key)) {
            if (thisNode.has(value)) {
                alert("Please check your Connections, you might entered two identical connections!");
                return false;
            }
